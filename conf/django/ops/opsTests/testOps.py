...
def checkStatus(self, response):
    ...
    self.assertEqual(
        response.status_code, 200, "SERVER ERROR: %s" % str(response.status_code)
    )
...
class createLayerTests(TestCase):
    def test_createLayer_status(self):
        ...
        self.assertEqual(
            self.models.layers.objects.filter(name__exact="test").count(), 1
        )
...
class deleteLayerTests(TestCase):
    ...
    def test_deleteLayer(self):
        ...
        self.assertEqual(
            self.models.layers.objects.filter(
                name__exact="fixtureTest", deleted=True
            ).count(),
            1,
        )
...
class createLayerPointsTests(TestCase):
    def test_createLayerPoints(self):
        ...
        request = self.factory.post(
            "create/layer/points", {"app": "rds", "data": jsonStr}
        )
        # Get the response
        ...
        self.assertEqual(
            self.models.layer_points.objects.filter(
                point_path_id__in=[4, 5, 6], user_id__username="admin", layer_id=1
            ).count(),
            3,
        )
...
class deleteLayerPointsTests(TestCase):
    ...
    def test_deleteLayerPoints_start_stop_point_path_id(self):
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "delete/layer/points", {"app": "rds", "data": jsonStr}
        )
        # Get the response
        ...
        self.assertEqual(
            self.models.layer_points.objects.filter(
                layer_id=3, point_path_id__in=[1, 3]
            ).count(),
            2,
        )
...
class deleteBulkTests(TestCase):
    ...
    def test_deleteBulk_onlyLayerPoints(self):
        ...
        self.assertEqual(
            self.models.layer_points.objects.filter(
                point_path_id__segment__name="11111111_01"
            ).count(),
            0,
        )
        ...
        self.assertEqual(
            self.models.segments.objects.filter(name="11111111_01").count(), 1
        )
    ...
    def test_deleteBulk_all(self):
        ...
        self.assertEqual(
            self.models.seasons.objects.filter(name="fixtureTest").count(), 0
        )
...
class releaseLayerGroupTests(TestCase):
    ...
    def test_releaseLayerGroup(self):
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "release/layer/group", {"app": "rds", "data": jsonStr}
        )
        # Get the response
        ...
        self.assertTrue(
            self.models.layer_groups.objects.filter(name="fixtureTest").values_list(
                "public", flat=True
            )[0]
        )
...
class getFrameClosestTests(TestCase):
    ...
    def test_getFrameClosest(self):
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "get/frame/closest", {"app": "rds", "data": jsonStr}
        )
        # Get the response
...
class getSegmentInfoTests(TestCase):
    ...
    def test_getSegmentInfo(self):
        ...
        # Set the json string
        jsonStr = (
            '{ "properties": { "segment": "11111111_01", "season": "fixtureTest" } }'
        )
...
class getUserProfileDataTests(TestCase):
    def test_getUserProfileData(self):
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "get/user/profile/data", {"app": "rds", "data": jsonStr}
        )
        # Get the response
...
class alterUserPermissionsTests(TestCase):
    def test_alterUserPermissions(self):
        ...
        User.objects.create_user(
            username="testDefault", email="test@default.test", password="testDefault"
        )
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "alter/user/permissions", {"app": "rds", "data": jsonStr}
        )
        # Get the response
...
class queryTests(TestCase):
    ...
    def test_query(self):
        ...
        # Create the request from the above app & jsonStr
        request = self.factory.post(
            "query", {"query": "SELECT id FROM rds_point_paths"}
        )
        # Get the response
